...
use_index = int(1.0 * len(poems))
poems = poems[:use_index]
...
if __name__ == "__main__":
    train_encoded = [
        torch.tensor(encode(poem), dtype=torch.long) for poem in train_poems
    ]
    ...
    if os.path.exists("NanoPoet_model.pt"):
        ...
        train(model, train_loader, eval_loader, optimizer, device, epochs=10)
